...
import { useAuthorProfile, useFollowing } from '@/hooks/useUsers';
...
export default function AuthorFollowingPage() {
  const { username = '' } = useParams();
  const [page, setPage] = useState(1);
  const { data: author, isLoading: authorLoading } = useAuthorProfile(username);
  const { data, isLoading, isError, refetch } = useFollowing(author?.id, page);
  ...
  return (
    <div className="container py-10">
      ...
      <div className="mb-8 flex items-center gap-2">
        ...
        <h1 className="text-2xl font-bold">{author.name} follows</h1>
      </div>

      {isLoading ? (
        <PostListSkeleton count={6} />
      ) : isError ? (
        <QueryErrorState title="Couldn't load following" onRetry={() => refetch()} />
      ) : !data?.users?.length ? (
        <EmptyState icon={UserCheck} title="Not following anyone yet" />
      ) : (
